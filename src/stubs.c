...
CAMLprim value mirage_pci_attach(value v_devname) {
    CAMLparam1(v_devname);
    const char *v = String_val(v_devname);
    CAMLreturn(Val_int(pci_attach(v)));
}

CAMLprim value mirage_enable_dma(value v_fd) {
    CAMLparam1(v_fd);
    pci_enable_dma(Int_val(v_fd));
    CAMLreturn(Val_unit);
}

CAMLprim value mirage_map_region(value v_fd, value v_region_index) {
    CAMLparam2(v_fd, v_region_index);
    CAMLlocal1(v_result);
    size_t size;
    void *buffer = pci_map_region(Int_val(v_fd), Int_val(v_region_index), &size);
    v_result = caml_ba_alloc_dims(FLAGS, 1, buffer, size);
    CAMLreturn(v_result);
}

CAMLprim value mirage_allocate_dma(value v_size) {
    CAMLparam1(v_size);
    CAMLlocal1(v_result);
